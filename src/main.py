...
app = Flask(__name__)
_MY_GUESS_TRACKER = {}
_TEAM_NOT_PROVIDED = "Please put your team name and password into config.py and start again."
...
def game_status_received(err, data):
    if err is not None:
        ...
        print()
        print(Fore.YELLOW + "GameId:", data["gameId"], Fore.YELLOW + "RoundId:", data["roundId"], Fore.YELLOW + "State:", data["state"], Fore.YELLOW + "#Participants:", len(data["participants"]))
        print("------------------------------------------------------------------------------------------\n")
        ...
        for participant in data["participants"]:
            if "name" in participant and participant["name"] == team_name and participant["joinedInThisRound"] == True:
                joined_status = True
        ...
        if state == 'joining':
            ...
            if not joined_status:
                print(Fore.YELLOW + "Oho, I have missed the joining phase, let me wait till the next round starts")
            elif not alive_status:
                print(Fore.RED + "I am dead, waiting to respawn in next round...:(")
            else:
                my_next_guess = apply_guess(data["gameId"], data["roundId"], data["secretLength"], data["participants"], _MY_GUESS_TRACKER)
                if my_next_guess is not None and len(my_next_guess["guesses"]) > 0:
                    ...
                    if err_guess is not None:
                        ...
                        if "guesses" in data_guess and data_guess["guesses"] is not None:
                            for item in data_guess["guesses"]:
                                if "score" in item:
                                    total_score_current_guess += item["score"]
                        print(Fore.GREEN + "Guess Successful : Score {}\n".format(total_score_current_guess))
