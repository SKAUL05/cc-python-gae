...
def apply_guess(*args, **kwargs):
    ...
    for index in range(len(kwargs.get('participants',0))):
        if not kwargs.get('participants',{})[index]['isAlive'] or kwargs.get('participants',{})[index]['name'] == _TEAM.upper():
    ...
    for random_guess in range(5):
        try:
            participant = kwargs['participants'][random.randint(0, total_participants-1)]
